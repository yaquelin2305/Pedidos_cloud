...
/**
 * Traduce las excepciones del dominio a una respuesta JSON uniforme, con la misma estructura
 * de ApiErrorResponse que ms-pedidos360-catalog. El 401 de token ausente/invalido lo resuelve
 * el entry point de seguridad; el 403 de @PreAuthorize y el de ForbiddenOrderAccessException
 * se manejan aqui porque los lanza el controlador/servicio y no llegan al filtro de seguridad.
 */
@RestControllerAdvice
public class GlobalExceptionHandler {

    @ExceptionHandler(OrderNotFoundException.class)
    public ResponseEntity<ApiErrorResponse> handleNotFound(OrderNotFoundException ex, WebRequest request) {
    ...
    public ResponseEntity<ApiErrorResponse> handleStockUnavailable(StockUnavailableException ex,
            WebRequest request) {
        return build(HttpStatus.CONFLICT, ex.getMessage(), request);
    }

    @ExceptionHandler(ForbiddenOrderAccessException.class)
    public ResponseEntity<ApiErrorResponse> handleForbiddenAccess(ForbiddenOrderAccessException ex,
            WebRequest request) {
